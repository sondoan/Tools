...
filtered=/export/home/scratch2/doan/brendan/PlosOne-Extra1
...
do
    echo $i
    #awk -F '\t' '{print $1 "\t" $2 "\t" $4 "\t" $5}' $i|/bin/egrep -i -f $respiratory_synd|/bin/grep -v -i -f $exclude1 > $filtered/$i.parsed
