...
filtered=/export/home/scratch2/doan/brendan/PlosOne-Eysenbach-http

cd $brendan
for i in `ls *.gz`
do
    echo $i
    awk -F '\t' '{print $1 "\t" $2 "\t" $4 "\t" $5}' $i|/bin/egrep -i "h1n1|swineflu|swine flu"|grep -v "http" > $filtered/$i.parsed
done
