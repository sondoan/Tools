...
def main():
    ...
    while next <= time2:
        ...
        next11 = time.strptime(next1,"%Y-%m-%d")
        weeknum = time.strftime("%U",next11)
        weeknum1 = int(weeknum) + 1
        next12 =next.strftime('%m/%d/%Y')
        ...
        print str(weeknum1) + "\t" + str(next12) + "\t" + count_num.strip()
        temp_file.close()
